...
The function \\(\mathrm{Li}(x)\\) is also called the _logarithmic integral_, and will turn out to be the main term in our formula for \\(J(x)\\), and a great approximation for the number of primes less than \\(x\\) in general. I've [mentioned it before]({{<ref "posts/2013-11-24-are-primes-independent.md">}}), and interpreted there \\(1/\log t\\) as the density of the primes "near" \\(t\\). This means we need to sum, or rather integrate, along all values of \\(t\\) to obtain the total number of primes. Those working with (continuous) probabilities will be familiar with the concept.
